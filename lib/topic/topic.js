...
export function show (topic) {
  ...
  // Build article's comments, fetch them
  // and render to section.app-content
  if(topic.subtopic){
...
page(forumRouter('/topic/:id'), setDemosFilter, user.optional, findForum, findTopic, findTopics, (ctx, next) => {
...
function setDemosFilter (ctx, next) {
  if (!config.demosLayout) return next();
  topicFilter.setFilter({sort: 'parent-and-static-order'});
  next();
}
