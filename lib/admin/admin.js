...
page(forumRouter('/admin/topics/create'), clearTagStore, findParentTopics, findAllTags, ctx => {
  ctx.sidebar.set('topics');
  // render new topic form
  let form = new TopicForm(null, ctx.forum, ctx.tags, ctx.parents);
...
function findParentTopics (ctx, next) {
  if (!config.demosLayout) return next();
...
page(forumRouter('/admin/topics/:id'), clearTagStore, findParentTopics, findAllTags, findTopic, ctx => {
  // force section for edit
  // as part of list
  ctx.sidebar.set('topics');

  // render topic form for edition
  let form = new TopicForm(ctx.topic, ctx.forum, ctx.tags, ctx.parents);
  form.replace('.admin-content');
