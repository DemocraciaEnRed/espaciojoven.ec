...
var topicListKeys = [
  ...
  'publishedAt deletedAt votable subtopic clauseTruncationText links author authorPictureUrl authorUrl keywords  summary'
].join(' ');

var topicKeys = topicListKeys
              + ' '
              + 'clauses source state upvotes downvotes abstentions';
...
app.get('/topic/parents', function (req, res) {
  ...
  api.topic.search(query, function(err, parentTopics) {
    ...
    log('Serving parent topics %j', pluck(parentTopics, 'id'));

    res.json(parentTopics.map(function(topicDoc){
      return exposeTopic(topicDoc, req.user, topicListKeys);
    }));
  });
});
