...
var topicListKeys = [
  'id topicId title mediaTitle clauses status open closed public draft deleted forum',
  'tag participants voted createdAt updatedAt closingAt parent order styles cover party partyUrl',
  'publishedAt deletedAt votable subtopic clauseTruncationText links author authorPictureUrl authorUrl keywords children'
].join(' ');
...
app.get('/topic/parents', function (req, res) {
  log('Request /topic/parents');

  var query = { parent: null, deletedAt: null };

  api.topic.search(query, function(err, parentTopics) {
    if (err) return _handleError(err, req, res);

    parentTopics = parentTopics.filter(function(topic) {
      if (topic.public) return true;
      if (req.query.draft && topic.draft) return true;
      return false;
    });

    function children(parent, callback) {
      api.topic.search({ parent: parent.id }, function (err, childrenTopics) {
        if (err) return callback(err);
        parent.children = childrenTopics;
        callback(null, parent);
      });
    }

    async.map(parentTopics, children, function (err, parents) {
      if (err) return _handleError(err, req, res);

      log('Serving parent topics %j', pluck(parents, 'id'));

      res.json(parents.map(function(topicDoc){
        return exposeTopic(topicDoc, req.user, topicListKeys);
      }));
...
app.post('/topic/create', restrict, hasAccess, function (req, res, next) {
  ...
  api.topic.create(req.body, function (err, topic) {
    ...
    var keys = [
      ...
      'publishedAt deletedAt votable bodyTruncationText links author authorUrl keywords'
    ].join(' ');
    res.json(exposeTopic(topic, req.user));
  });
});
