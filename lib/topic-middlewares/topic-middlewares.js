...
export function findTopics(ctx, next) {
  ...
  let query = {};
  if (config.multiForum) query.forum = ctx.forum.id;
  if (config.demosLayout) query.parent = ctx.topic.parent || ctx.topic.id;
...
/**
 * Load only parent topics
 */

export function findParents(ctx, next) {
  if (!config.demosLayout) {
    throw new Error('Only available on DEMOS');
  }

  topicStore.findParents().then(topics => {
    ctx.topics = topics;
    next();
  })
}
