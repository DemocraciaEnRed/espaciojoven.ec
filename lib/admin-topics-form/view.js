...
export default class TopicForm extends FormView {
  ...
  constructor(topic, forum, tags, parents) {
    super();
    this.setLocals(topic, forum, tags, parents);
  ...
  setLocals(topic, forum, tags, parents) {
    if (topic) {
  ...
  postserialize(data) {
    ...
    data.subtopic = data.subtopic || false;
    data.author = data.topicAuthor;
    data.keywords = this.topic ? this.topic.keywords : [];
    data.parent = data.parent || null;

    data.styles = {};
    data.styles.fgColor = data.fgColor;
    data.styles.bgColor = data.bgColor;
    delete data.fgColor;
    delete data.bgColor;
  ...
  renderToggles() {
    var votableToggle = new Toggle();
    votableToggle.label('Yes', 'No');
    votableToggle.name('votable');
    votableToggle.value(this.topic == undefined || this.topic.votable === undefined ? true : !!this.topic.votable);
    this.find('.votable-toggle').append(votableToggle.el);

    var subtopicToggle = new Toggle();
    subtopicToggle.label('Yes', 'No');
    subtopicToggle.name('subtopic');
    subtopicToggle.value(this.topic == undefined || this.topic.votable === undefined ? true : !!this.topic.subtopic);
    this.find('.subtopic-toggle').append(subtopicToggle.el);
  }

  renderKeywords() {
    const keywords = this.topic ? this.topic.keywords : []
    mount(
      <TagsInput tags={keywords} handleChanged={this.keywordsChanged.bind(this)} />,
      this.find('input#keywords').parent()[0]
    )
  }
