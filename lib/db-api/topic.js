...
exports.all = function all(params, fn) {
  ...
  var query = Topic
    .find({ deletedAt: null })
    .select('id topicId mediaTitle clauses tag participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author authorPictureUrl authorUrl forum parent order keywords subtopic styles party partyUrl summary')
    .populate('tag', 'id hash name color image');

  if (params.forum) query = query.where({forum: params.forum});
  if (params.parent) query = query.or([
    {_id: params.parent},
