...
var summaryValidator = [
    { validator: minSummaryValidator }
  , { validator: maxSummaryValidator }
];

function minSummaryValidator(summary) {
  return summary.length;
}

function maxSummaryValidator(summary) {
  return summary.length <= 230;
}
...
var TopicSchema = new Schema({
    ...
    author: { type: String },
    authorPictureUrl: { type: String, validate: validators.isURL({ skipEmpty: true }) },
    authorUrl: { type: String, validate: validators.isURL({ skipEmpty: true }) },
    party: { type: String },
    partyUrl: { type: String, validate: validators.isURL({ skipEmpty: true }) },
    ...
    parent: { type: ObjectId, ref: 'Topic' }, // If it's null, its the parent
    order: { type: Number, default: 0 }, // For children
    keywords: { type: [String] },
    subtopic: { type: Boolean, required: true, default: false },
    styles: { type: Schema.Types.Mixed },
    cover: { type: String, validate: validators.isURL({ skipEmpty: true }) },
    summary: { type: String, validate: summaryValidator,  trim: true }
});
